...
| Активні оголошення конкурсів на вакантні посади | `activeVacancyAnnouncements` у `src/data/vacancies.ts`, `activeVacancyAnnouncementsEn` у `src/data/vacanciesEn.ts`, `src/pages/vacancies.astro`, `src/pages/en/vacancies.astro` | Підписане офіційне "Оголошення про конкурс"; до підписання допускається підготовлений запис із приміткою, що файл буде додано після підписання |
| Архів конкурсів на вакантні посади | `vacancyArchive` у `src/data/vacancies.ts`, `vacancyArchiveEn` у `src/data/vacanciesEn.ts`, `scripts/check-vacancy-archive.mjs` | `order-competition.pdf`, офіційне "Оголошення про конкурс", накази про учасників і результати |
...
Поточні конкурси мають два стани: активне оголошення і архівний запис.

Активне оголошення використовується для щойно оприлюдненого або підготовленого до оприлюднення конкурсу, коли повний архів із трьома наказами ще не сформований. Воно зберігається окремо від архіву:
...
2. У полі `positions` зазначайте саме посади, а не прізвища осіб. Якщо одна посада належить до відділу, додавайте посилання на відділ у тому самому блоці позиції.
...
5. Не додавайте посилання на `order-competition.pdf`, `order-participants.pdf` або `order-results.pdf`, доки цих файлів немає. Активне оголошення не повинно створювати битих посилань.

6. Коли підписаний файл "Оголошення про конкурс" з'являється раніше за накази про учасників і результати, його можна додати як `document` активного оголошення, використовуючи шлях без префікса `public`.
...
9. Якщо конкурс спочатку був активним оголошенням, після появи трьох стандартних PDF-файлів перенесіть його з `activeVacancyAnnouncements` / `activeVacancyAnnouncementsEn` до архівних масивів `competitions` у двох мовних файлах і приберіть активний запис.
